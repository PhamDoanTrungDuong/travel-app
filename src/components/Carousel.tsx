  ...
  return (
    <div className="max-w-[1240px] mx-auto px-4 py-16 relative flex justify-center items-center">
      {sliderData.map((item, idx) => {
        return (
          <div className={idx == slide ? "opacity-100" : "opacity-0"} key={idx}>
            <BsArrowLeftSquareFill
              onClick={prevSlide}
              className="absolute hover:scale-105 duration-200 top-[50%] text-3xl text-white cursor-pointer left-8"
            />
            <BsArrowRightSquareFill
              onClick={nextSlide}
              className="absolute hover:scale-105 duration-200 top-[50%] text-3xl text-white cursor-pointer right-8"
            />
            {idx === slide && (
              <img
